...
class Menu {
	...
	public function render(array $attrs = NULL, array $items = NULL)
	{
		...
		// a faire olivier
		// meilleure gestion des dropdown pour multiniveaux
		
		$attrs['class'] = empty($attrs['class']) ? 'level-'.$i : $attrs['class'];//.' level-'.$i;
		if($i == 2){
			//$menu = '<a href="#" class="dropdown-toggle" data-toggle="dropdown"><b class="caret"></b></a>';
		...
		foreach ($items as $key => $item)
		{
			...
			$menu .= '<li'.$classes.'>'.HTML::anchor($item['url'], $item['title'],$item['attribute']);
			if ($has_children)
			{
