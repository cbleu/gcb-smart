...
abstract class Base_DB_MySQL_Expression implements DB_SQL_Expression_Interface {
	...
	public function prepare_alias($expr) {
		...
		// modif olivier ajout de * et . pour la gestion de oscrud
	...
	public function prepare_identifier($expr) {
		...
		foreach ($parts as &$part) {
			//echo "passe 6=".$part."<br/>";
			// modif olivier ajout traitement particulier pour set_relation * pour oscrud
			if($part=='*'){
				$part = trim(preg_replace('/[^a-z0-9*$_ ]/i', '', $part));
			}
			else{
				// modif olivier ajout * pour oscrud
				$part = static::_OPENING_QUOTE_CHARACTER_ . trim(preg_replace('/[^a-z0-9*.$_ ]/i', '', $part)) . static::_CLOSING_QUOTE_CHARACTER_;
			}
		}
		$expr = implode('.', $parts);
		
		//echo "result expr=".$expr."<br/>";
		return $expr;
	}
