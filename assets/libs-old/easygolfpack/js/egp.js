...
function initWizard()
{
	...
	// Initialisation du wizard (formwizard.js)
	var myform=new formtowizard({
		...
		onpagechangestart:function($, i, $fieldset){
			setTimeout(function() {
				// //////////////////////////////////////////////////////
				// // ECRAN DATE ////////////////////////////////////////
				if($("#date_fieldset").attr("style").indexOf("visible") !== -1) {	// affichage de l'écran de parcours et horaire
				...
				//////////////////////////////////////////////////////
				// ECRAN PARCOURS ////////////////////////////////////
				if($("#parcours_fieldset").attr("style").indexOf("visible") !== -1) {	// affichage de l'écran de parcours et horaire
				...
				//////////////////////////////////////////////////////
				// ECRAN JOUEURS /////////////////////////////////////
				if($("#players_fieldset").attr("style").indexOf("visible") !== -1) {	// affichage de l'écran des joueurs
					// console.log("players_fieldset");
				...
				//////////////////////////////////////////////////////
				// ECRAN RESUME //////////////////////////////////////
				if($("#confirmation_fieldset").attr("style").indexOf("visible") !== -1) {	// affichage de l'écran de resumé
					DisplayDigest();
				}// ECRAN RESUME
			}, 600);
		}
	...
	$("#date_resa").change(function(){
		...
		// var textfield_date = $.datepicker.parseDate("dd/mm/yy", $(this).val());

		// if(textfield_date < currentDate) {
			// $( "#datepicker" ).datepicker( "setDate", currentDate );
			// $(this).val($.datepicker.formatDate('dd/mm/yy', currentDate));
		// }else if(textfield_date > maxDate){
			// $( "#datepicker" ).datepicker( "setDate", maxDate );
			// $(this).val($.datepicker.formatDate('dd/mm/yy', maxDate));
		// }
		
		// if($(this).val() == "") {
		// 	$("#heure_resa").prop('disabled', true);
		// 	return;
		// }
		// $("#heure_resa").prop('disabled', false);
	...
	$('#wizard_form').ajaxForm({
		beforeSubmit: function(data, form, options) {
			...
			options['url'] = "/reservationajax/add";
...
function fill_DOM_with_Intervals(lastslot)	// rempli les slots horairesdu DOM avec le tableau des intervalles
{
	...
	for(var i = 0; i < hoursIntervals.length; i++) {
		...
		if (thattime < thattimelast){
			$("#heure_resa optgroup#nb_trous_optgroup").append('<option value="' + hoursIntervals[i] + '">' + hoursIntervals[i] + " " + hoursPlayers[i] + '</otpion>');
		}
	}
...
function load_user_available_Intervals()	// ajax: slots dispo pour ce joueur à partir de maintenant
{
					...
					while (len--) {
						if(hoursIntervals[len] == horaires_reserves[i][0]) {
							...
							if(horaires_reserves[i][1] == 0) {		// plus de place
								...
								hoursPlayers[len] = " (dernière place disponible)";
	...
	{
		dataType: "json",
		url: '/reservationajax/userblockedtimes?day_date='+ $("#date_resa").val(),
		success: function( data ) {
...
function DisplayDigest(){
	...
	{
		type: "POST",
		url: "/reservationajax/wizarddigest",
		dataType: "json",
	...
	$('#form').ajaxForm({
		beforeSubmit: function(data, form, options) {
			...
			if(vars.isLogged){
				...
				options['url'] = "/reservationajax/addpublic";
			}
			$("#loading_div").height($("#form").height());
			$("#form").hide();
			$("#loading_div").show();
...
function initScheduler(){
	...
	scheduler.renderEvent = function(container, ev) {
		...
		// if(ev.evt == "evenement" && ((ev.start_date.getTime() + 60*10*1000) != ev.end_date.getTime())) {
		// 	// var background = 'background:"' + ev.color + '";';
		// 	// var textColor  = 'color:"' + ev.textColor + '";';
		// 	// var html = '';
		// 	// html += '<div';
		// 	// html += 	' class=" dhx_event_move dhx_title evenement" ';
		// 	// html += 	' style="' + background + textColor;
		// 	// html += ' >';
		// 	// html += '<span style="font-weight:normal;">' + ev.text + '</span>';
		// 	// html += '</div>';	// class="dhx_event_move dhx_title
		// 	// container.innerHTML = html;
		// 	// return true; // required, true - we've created custom form; false - display default one instead
		// 	// container.className += "evenement";
		// 	return false;
		// }
	...
	// getPlayersBySlotAt(scheduler.getState().date);
...
function resize_calendar(){
	...
	$("#scheduler_here").height(document_height - menu_div_height - footer_div_height - 40);
}	// resize_calendar
...
function loadUserBlockTime(thatDate){
	...
	$.getJSON('/reservationajax/userblockedtimes?day_date='+shortStrDate, function(data){
		$.each(data, function(key, val) {
...
function detectUserPlayTimespan(event){
	// if(scheduler.checkInMarkedTimespan(scheduler.getEvent(event), "user_play")){
...
function startEditEventForm(ev){
	...
	$.ajax({
		type: "POST",
		url: "/reservationajax/loaddetailsform",
		dataType: "json",
		data: {
...
function ReleaseResaProvi(formdata){
	...
	$.ajax({	// requete ajax pour supprimer les reservation provisoires
		...
		type: "POST",
		url: "/reservationajax/cancelresaprovi",
		dataType: "json",
		data:  formdata.serialize(),
